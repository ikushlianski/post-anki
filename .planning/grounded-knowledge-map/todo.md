...
This plan ran fully autonomously (no `AskUserQuestion` — unattended, per explicit instruction). Every fork resolved with a recommended default per the recommended-default rule; full reasoning is in `spec.md`'s "Decisions made autonomously" (15 entries). This file logs what's still genuinely open, plus manual/post-deploy steps.

## Decisions to make

Nothing left open from the original plan — every fork identified during
planning has a committed default logged in `spec.md`. One new decision
surfaced during Phase 1 implementation itself (not anticipated in planning):

- **Should the level picker's pre-inclusion bias be restored for
  research-triggered curricula?** Phase 1's approval gate introduces a real
  async gap between creation and generation that the level picker's chosen
  value doesn't currently survive (see build log's deviation #1) — every
  research-triggered curriculum's topics now start unincluded regardless of
  level picked, same as the bare-name path already behaved before this
  build. Restoring the old docUrl+level-picker bias would need a new
  `curricula.preferredLevel` nullable column (small, additive, same shape as
  every other Phase 1/2/3/4 schema addition) plumbed through the approval
  step. Left as a genuine product-preference call, not implemented, since it
  wasn't in spec.md's data model and expanding schema scope on my own
  judgment felt like the wrong default to pick unattended.

## To review / clarify

- **Migration sequencing across four phases — RESOLVED for Phase 1.** Generated incrementally: one migration (`0013_wooden_lake.sql`) covering only `sources.approval_status`. Whoever builds Phase 2/3/4 should generate their own migration(s) against the schema as it stands after this one, not regenerate from scratch.
- **Phase 3 and Phase 4 share files** (`probe-session.generate.ts`, `.repo.ts`, `.service.ts`) — `spec.md`'s Implementation Phases table calls for building Phase 3 before Phase 4 within this same plan to avoid touching `ScopeContext`/`ScopeTopic` twice in an uncoordinated order. If `/moonshine` or another builder parallelizes phases across subagents, this ordering constraint must be respected (sequential handoff on those specific files, not simultaneous edits). Additionally: whoever builds Phase 3 will touch `curriculum-research-plan.ts` for the new `tags` field — that file was also touched in Phase 1 (see build log's deviation #3, a `strictOrder` schema bug fix); small, non-conflicting rebase surface, flagging so it isn't a surprise.
- **Whether to author real Playwright scenarios in `verification-repo` — RESOLVED for Phase 1: not done.** Authoring new scenarios there is a separate `/write-playwright-tests`-style effort. Instead used real `curl` against a running API plus ad hoc real-browser Playwright (chromium launched directly, screenshotted, clicked through the actual remove/add/approve flow) — see the build log above for exactly what was driven and observed. The existing registered `study-technology-doc-url` test in `verification-repo` will now fail red (see build log) until someone updates it for the gated flow — that update is a good candidate for whoever picks up Phase 1's e2e authoring properly.

## Manual steps

- After all four phases' `schema.ts` edits are in place, run `npm run db:generate:api` then `npm run db:migrate:api` against the local dev database before testing anything in this plan.
- No new environment variables, secrets, or API keys — the general trusted-source search and the bounded crawl both reuse existing `OPENROUTER_API_KEY` wiring and plain `fetch()`, same as every existing grounding call in this codebase.
...
- Create a curriculum by bare technology name (no docUrl) for a real, well-documented technology and confirm the candidate list on the approval screen actually reflects that technology's real official docs, not a hallucinated URL.
- Create a curriculum for a genuinely obscure name and confirm the empty-candidates warning/override path actually appears rather than the app silently generating anyway.
...
- Spot-check that a curriculum created via the Phase 1 empty-source override shows the new "ungrounded" notice in quiz mode, and that a normally-sourced curriculum does not show it.
- Decide whether existing in-progress curricula being routed through the pre-assessment screen once (see Phase 2 build log) is the desired behavior for real users, or whether a backfill (`preAssessmentCompletedAt = now()` for every curriculum already `confirmed` before this migration) should ship alongside it instead.
- **New from Phase 3:** decide whether `POST /tags` (resolve-or-create by name) should be folded into a documented addition to spec.md's endpoint list, or left as an implementation-time extension — it's real, tested, and required for the tag picker to function, but wasn't in the plan's literal file/endpoint list.
- **New from Phase 3:** a near-duplicate AI-suggested tag name that isn't exact-after-normalization (e.g. "Perf" vs "Performance") will still create a second tag, per architecture.md's own accepted-limitation note — worth a periodic manual glance at the `tags` table for accumulating near-duplicates as more curricula get generated.
- **New from Phase 4:** decide whether topic-scope quizzes should keep starting exactly at the replenish floor (`scaleTopicQuizTotal`'s own floor and `REPLENISH_FLOOR` are both 10 — see Phase 4 build log below), which means a small topic (few gaps) fires its first replenish after just one answer rather than only once a "large" session is genuinely running low. Not a bug against any scenario as written, but worth a product-preference check against real usage.
