...
## Extension: grounded per-option explanations, citation validation, and uncapped batch sizing

*(added by `probe-quiz-grounded-explanations`, a same-service follow-up to the plan above — no new services, no new async boundaries)*

**Per-option explanations travel alongside `options`, shuffled in lockstep.** The generation agent now returns one `{ text, citationUrl }` per option, in the same order as `options`. When `buildQuestionRows` reindexes `options` via the existing `reindexOptions` deriver, it applies the *same permutation* to `optionExplanations` via a new sibling deriver, `reindexParallelArray(items, permutation)` (`packages/core/src/probe-session/shuffle.ts`) — added alongside `reindexOptions`, not a change to its signature. Reveal timing mirrors the existing correct-answer gate exactly: `optionExplanations` is only populated in the DTO once `outcome !== null` (`probe-session.repo.ts`'s `rowToSessionQuestion`), and is nullable throughout the shared schema so pre-migration rows degrade to "no explanation available" instead of breaking.

**The "never a fabricated citation" guarantee is built by provenance, never by text-mining model-generated prose — this is the core anti-hallucination mechanism.** `getCurriculumCitableUrls` (`apps/api/src/curriculum/curriculum.repo.ts`) builds a known-URL allowlist from exactly two provenance-verified sources, reusing the curriculum's already-persisted `sources` rows with no new fetch:
1. Every source row's own `value` field, when it parses as an absolute `http(s)` URL, regardless of `kind`.
2. URLs regex-extracted (`extractUrls`, a pure deriver in `packages/core/src/probe-session/option-explanations.ts`) from `fetchedText` — but **only** for rows whose `kind` is `llms_txt`, `link`, or `text` (genuine fetched-or-pasted document content). `web_research`-kind rows are deliberately excluded from this step: their `fetchedText` is `gatherTechResearchGrounding`'s model-*written* search summary, not a fetched document, and extracting "URLs" out of prose the model itself typed would trust exactly the kind of unverified claim this feature exists to distrust.

For the live web-search fallback branch (curriculum has no usable persisted sources), the allowlist instead reuses the `citations` array `gatherProbeGrounding`'s `webGround` already collects from OpenRouter's structural `url_citation` annotations — a channel the search API verifies separately from the prose it also returns. `extractUrls` is never run over that branch's own generated text either.

After the agent returns structured output, `sanitizeOptionExplanations(explanations, knownUrls)` — a pure, cheap post-generation filter, no second LLM call, no live HTTP call — nulls any `citationUrl` not present in that allowlist. This is fail-closed *per option*, not per question: one fabricated URL only nulls that one field; the explanation text and the rest of the question persist. A thin/no-grounding curriculum produces an empty allowlist, which means every citation nulls out but questions and explanations still generate from the model's general knowledge — the same empty-grounding fallback the base plan above already has for question content.

**Batch size scales with what there is to test, uncapped.** `scaleTopicQuizTotal(gapCount, floor)` (`packages/core/src/probe-session/quiz-size.ts`) replaces the flat `TOPIC_TARGET` constant for topic-scope generation; module-scope generation drops its `MAX_TOTAL` ceiling and keeps only the floor. The explicit "Generate Probing Questions" button on the topic page (`probe-session-quiz.tsx`) is the UX mitigation for the resulting cost/latency variance — quiz generation no longer fires automatically on page mount; the learner opts into it, and a loading state covers the full generation call.
